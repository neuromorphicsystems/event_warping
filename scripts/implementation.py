...
TMAX                = 20e6
RATIO               = 0.00001
imgpath             = "/home/samiarja/Desktop/PhD/Code/orbital_localisation/img/weight_map"
READFROM            = "/home/samiarja/Desktop/PhD/Code/orbital_localisation/data/es/NADIR/"
SAVEFILETO          = "/home/samiarja/Desktop/PhD/Code/orbital_localisation/img/"
...
def alpha_1(imageOfWarpedEvents):
    """
    Input:
...
def alpha_2(imageOfWarpedEvents, section: int):
    """
    Input:
    imageOfWarpedEvents: A 2D numpy array representing the warped image, where pixel values represent the event count.
...
for iVely in range(0,nVel):
    ...
    for iVelx in range(0,nVel):
        ...
        iwe         = event_warping.accumulate((width, height), events, velocity)
        vx          = np.abs(fieldx*t)
        vy          = np.abs(fieldy*t)
        ...
        if (fieldx*t >= 0.0 and fieldy*t >= 0.0 and np.abs(fieldx*t)<=width and np.abs(fieldy*t)<=height) or (fieldx*t <= 0.0 and fieldy*t <= 0.0 and np.abs(fieldx*t)<=width and np.abs(fieldy*t)<=height):
            fun_idx                 += 1
            corrected_iwe            = alpha_1(iwe.pixels)
        
        if (fieldx*t >= 0.0 and fieldy*t <= 0.0 and np.abs(fieldx*t)<=width and np.abs(fieldy*t)<=height) or (fieldx*t <= 0.0 and fieldy*t >= 0.0 and np.abs(fieldx*t)<=width and np.abs(fieldy*t)<=height):
            fun_idx                 += 2
            iwe.pixels              = mirror_image(iwe.pixels)
            corrected_iwe           = alpha_1(iwe.pixels)
        ...
        if (fieldx*t >= 0.0 and fieldy*t >= 0.0 and np.abs(fieldx*t)>=width and np.abs(fieldy*t)<=height) or (fieldx*t <= 0.0 and fieldy*t <= 0.0 and np.abs(fieldx*t)>=width and np.abs(fieldy*t)<=height) or ((((vy/vx)*width)-height)/(np.sqrt(1+(vy/vx)**2)) <= 0 and fieldx*t >= 0.0 and fieldy*t >= 0.0 and np.abs(fieldx*t)>=width and np.abs(fieldy*t)>=height) or ((((vy/vx)*width)-height)/(np.sqrt(1+(vy/vx)**2)) <= 0 and fieldx*t <= 0.0 and fieldy*t <= 0.0 and np.abs(fieldx*t)>=width and np.abs(fieldy*t)>=height):
            fun_idx                 += 3
            corrected_iwe            = alpha_2(iwe.pixels,1)

        if (fieldx*t >= 0.0 and fieldy*t >= 0.0 and np.abs(fieldx*t)<=width and np.abs(fieldy*t)>=height) or (fieldx*t <= 0.0 and fieldy*t <= 0.0 and np.abs(fieldx*t)<=width and np.abs(fieldy*t)>=height) or ((((vy/vx)*width)-height)/(np.sqrt(1+(vy/vx)**2)) > 0 and fieldx*t >= 0.0 and fieldy*t >= 0.0 and np.abs(fieldx*t)>=width and np.abs(fieldy*t)>=height) or ((((vy/vx)*width)-height)/(np.sqrt(1+(vy/vx)**2)) > 0 and fieldx*t <= 0.0 and fieldy*t <= 0.0 and np.abs(fieldx*t)>=width and np.abs(fieldy*t)>=height):
            fun_idx                 += 4
            corrected_iwe            = alpha_2(iwe.pixels,2)
        ...
        var = variance(corrected_iwe)
        # saveimg(corrected_iwe, var, fun_idx, t, fieldy, fieldx, imgpath, draw=True)
