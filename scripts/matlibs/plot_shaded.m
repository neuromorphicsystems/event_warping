...
y_min = min(Y);
y_max = max(Y);
V_alpha = 1*alpha_value*((Y-y_min)/y_max);
F = [1 2 3 4];
...
for i=1:numel(X)-1
    ...
    patch('Faces',F,'Vertices',V,'FaceColor',color_value,...
        'EdgeColor','none',...
        'FaceVertexAlphaData',A,'FaceAlpha','interp','AlphaDataMapping','none','HandleVisibility','off');
...
if nargin > minargs
    for i=(minargs+1):2:nargin
        ...
        switch PNAME
            case 'Alpha'
                validateattributes(PVALUE,{'numeric'},{'scalar','nonnan','finite','nonnegative','<=',0.1},mfilename,PNAME,i+1);
                alpha_value = PVALUE;
            case 'Color'
