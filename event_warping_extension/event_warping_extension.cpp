...
static PyObject* intensity_variance_weighted(PyObject* self, PyObject* args) {
    int32_t sensor_width;
    int32_t sensor_height;
    PyObject* raw_ts;
    PyObject* raw_xs;
    ...
    try {
        ...
        // f_1(x,y)
        if (corrected_velocity_x* (t1 - t0) >= 0 && corrected_velocity_y* (t1 - t0) >= 0 && corrected_velocity_x* (t1 - t0)<=sensor_width && corrected_velocity_y* (t1 - t0)<=sensor_height) {
            // condition 1
            for (std::size_t y = corrected_velocity_y* (t1 - t0); y < sensor_height; ++y) {
                for (std::size_t x = corrected_velocity_x* (t1 - t0); x < sensor_width; ++x) {
            ...
            // condition 2
            for (std::size_t y = corrected_velocity_y* (t1 - t0); y < sensor_height; ++y) {
                for (std::size_t x = 0; x < corrected_velocity_x* (t1 - t0); ++x) {
                    const auto determinant = y * corrected_velocity_x - x * corrected_velocity_y;
                    if (determinant >= minimum_determinant && determinant <= maximum_determinant) {
                        const auto value = cumulative_map[x + y * width];//*((((corrected_velocity_x* (t1 - t0))/(x + 1))));
                        const auto delta = value - mean;
                        mean += delta / static_cast<double>(count + 1);
                        m2 += delta * (value - mean);
                        ++count;
            ...
            // condition 3
            for (std::size_t y = 0; y < corrected_velocity_y* (t1 - t0); ++y) {
                for (std::size_t x = corrected_velocity_x* (t1 - t0); x < sensor_width; ++x) {
                    const auto determinant = y * corrected_velocity_x - x * corrected_velocity_y;
                    if (determinant >= minimum_determinant && determinant <= maximum_determinant) {
                        const auto value = cumulative_map[x + y * width];//*((((corrected_velocity_y* (t1 - t0))/(y + 1))));
                        const auto delta = value - mean;
                        mean += delta / static_cast<double>(count + 1);
                        m2 += delta * (value - mean);
                        ++count;
            ...
            // condition 4
            for (std::size_t y = (corrected_velocity_y* (t1 - t0)); y <= height; ++y) {
                for (std::size_t x = width; x <= ((corrected_velocity_x* (t1 - t0))+width); ++x) {
                    const auto determinant = y * corrected_velocity_x - x * corrected_velocity_y;
                    if (determinant >= minimum_determinant && determinant <= maximum_determinant) {
                        const auto value = cumulative_map[x + y * width];//*((corrected_velocity_x* (t1 - t0))/(-x+sensor_width+corrected_velocity_x* (t1 - t0)));
                        const auto delta = value - mean;
                        mean += delta / static_cast<double>(count + 1);
                        m2 += delta * (value - mean);
                        ++count;
            ...
            // condition 5
            for (std::size_t y = sensor_height; y < ((corrected_velocity_y* (t1 - t0))+sensor_height); ++y) {
                for (std::size_t x = (corrected_velocity_x* (t1 - t0)); x < sensor_width; ++x) {
                    const auto determinant = y * corrected_velocity_x - x * corrected_velocity_y;
                    if (determinant >= minimum_determinant && determinant <= maximum_determinant) {
                        const auto value = cumulative_map[x + y * width];//*((corrected_velocity_y* (t1 - t0))/(-y+sensor_height+corrected_velocity_y* (t1 - t0)));
                        const auto delta = value - mean;
                        mean += delta / static_cast<double>(count + 1);
                        m2   += delta * (value - mean);
                        ++count;
            ...
            // condition 6
            for (std::size_t x = 0; x < ((corrected_velocity_x* (t1 - t0))); ++x) {
                for (std::size_t y = (((-corrected_velocity_y* (t1 - t0))*(x))/(corrected_velocity_x* (t1 - t0)))+(corrected_velocity_y* (t1 - t0)); y < (corrected_velocity_y* (t1 - t0)); ++y) {
                    const auto determinant = y * corrected_velocity_x - x * corrected_velocity_y;
                    if (determinant >= minimum_determinant && determinant <= maximum_determinant) {
                        const auto value = cumulative_map[x + y * width];//*((corrected_velocity_x*(corrected_velocity_y* (t1 - t0)))/(((x+1)*corrected_velocity_y* (t1 - t0))+(y*corrected_velocity_x* (t1 - t0))-((corrected_velocity_x* (t1 - t0))*(corrected_velocity_y* (t1 - t0)))));
                        const auto delta = value - mean;
                        mean += delta / static_cast<double>(count + 1);
                        m2 += delta * (value - mean);
                        ++count;
            ...
            // condition 7
            for (std::size_t x = sensor_width; x < (((corrected_velocity_x* (t1 - t0)))+sensor_width); ++x) {
                for (std::size_t y = sensor_height; y < ((((-corrected_velocity_y* (t1 - t0))*x)/(corrected_velocity_x* (t1 - t0)))+(((corrected_velocity_y* (t1 - t0))*sensor_width)/(corrected_velocity_x* (t1 - t0)))+sensor_height+(corrected_velocity_y* (t1 - t0))); ++y) {
                    const auto determinant = y * corrected_velocity_x - x * corrected_velocity_y;
                    if (determinant >= minimum_determinant && determinant <= maximum_determinant) {
                        const auto value = cumulative_map[x + y * width];//*(((corrected_velocity_x*corrected_velocity_y* (t1 - t0)))/(-x*corrected_velocity_y* (t1 - t0)-y*corrected_velocity_x* (t1 - t0)+height*corrected_velocity_x* (t1 - t0)+(corrected_velocity_x* (t1 - t0)*corrected_velocity_y* (t1 - t0))));//*(((((corrected_velocity_x* (t1 - t0))*(corrected_velocity_y* (t1 - t0)))/(((corrected_velocity_x* (t1 - t0))*(corrected_velocity_y* (t1 - t0))-((corrected_velocity_x* (t1 - t0))*y)+((corrected_velocity_y* (t1 - t0))+width)-((corrected_velocity_y* (t1 - t0))*x)))));
                        const auto delta = value - mean;
                        mean += delta / static_cast<double>(count + 1);
                        m2 += delta * (value - mean);
                        ++count;
            ...
            // condition 9
            for (std::size_t x = sensor_width; x < (((corrected_velocity_x* (t1 - t0)))+sensor_width); ++x) {
                for (std::size_t y = (((corrected_velocity_y* (t1 - t0))*(-x+sensor_width+(corrected_velocity_x* (t1 - t0))))/(corrected_velocity_x* (t1 - t0))); y < (corrected_velocity_y* (t1 - t0)); ++y) {
                    const auto determinant = y * corrected_velocity_x - x * corrected_velocity_y;
                    if (determinant >= minimum_determinant && determinant <= maximum_determinant) {
                        const auto value = cumulative_map[x + y * width];//*((corrected_velocity_x* (t1 - t0))/((-x+sensor_width+(corrected_velocity_x* (t1 - t0)))));
                        const auto delta = value - mean;
            ...
            // condition 10
            for (std::size_t x = 0; x < (corrected_velocity_x* (t1 - t0)); ++x) {
                for (std::size_t y = sensor_height; y < sensor_height+(corrected_velocity_y* (t1 - t0))-(x*(corrected_velocity_y* (t1 - t0)))/(corrected_velocity_x* (t1 - t0)); ++y) {
                    const auto determinant = y * corrected_velocity_x - x * corrected_velocity_y;
                    if (determinant >= minimum_determinant && determinant <= maximum_determinant) {
                        const auto value = cumulative_map[x + y * width];//*((((corrected_velocity_x* (t1 - t0))/(x + 1))));
                        const auto delta = value - mean;
            ...
            // condition 11
            for (std::size_t x = 0; x < ((corrected_velocity_x* (t1 - t0))); ++x) {
                for (std::size_t y = sensor_height+(corrected_velocity_y* (t1 - t0))-((x)*(corrected_velocity_y* (t1 - t0)))/(corrected_velocity_x* (t1 - t0)); y < (((corrected_velocity_y* (t1 - t0)))+sensor_height); ++y) {
                    const auto determinant = y * corrected_velocity_x - x * corrected_velocity_y;
                    if (determinant >= minimum_determinant && determinant <= maximum_determinant) {
                        const auto value = cumulative_map[x + y * width];//*((((corrected_velocity_y* (t1 - t0))/(-(y)+height+corrected_velocity_y* (t1 - t0)))));
                        const auto delta = value - mean;
                        mean += delta / static_cast<double>(count + 1);
                        m2 += delta * (value - mean);
                        ++count;
        ...
        // f_2(x,y)
        else if (corrected_velocity_x* (t1 - t0) >= 0 && corrected_velocity_y* (t1 - t0) >= 0 && corrected_velocity_x* (t1 - t0)>=sensor_width && corrected_velocity_y* (t1 - t0)<=sensor_height){
        ...
        // f_3(x,y)
        else if (corrected_velocity_x* (t1 - t0) >= 0 && corrected_velocity_y* (t1 - t0) <= 0 && corrected_velocity_x* (t1 - t0)<=sensor_width && std::abs(corrected_velocity_y* (t1 - t0))<=sensor_height){
            for (std::size_t y = 0; y < height; ++y) {
                for (std::size_t x = 0; x < width; ++x) {
                    const auto determinant = y * corrected_velocity_x - x * corrected_velocity_y;
                    if (determinant >= minimum_determinant && determinant <= maximum_determinant) {
...
// static PyObject* eventmap(PyObject* self, PyObject* args) {
//     int32_t sensor_width;
//     int32_t sensor_height;
//     PyObject* raw_ts;
//     PyObject* raw_xs;
...
//         Py_END_ALLOW_THREADS;
//         return reinterpret_cast<PyObject*>(cumulative_map);
//     } catch (const std::exception& exception) {
//         PyErr_SetString(PyExc_RuntimeError, exception.what());
//     }
//     return nullptr;
...
static PyMethodDef event_warping_extension_methods[] = {
    ...
    // {"eventmap", eventmap, METH_VARARGS, nullptr},
    {"intensity_variance_weighted", intensity_variance_weighted, METH_VARARGS, nullptr},
